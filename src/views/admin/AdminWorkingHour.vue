<template>
  <div class="working-hour-management">
    ...
    <div class="hour-list" v-if="workingHours.length > 0">
      <div v-for="hour in workingHours" :key="hour.id" class="hour-item">
        <div class="hour-info">
          <h3 class="hour-day">{{ hour.day }}</h3>
          <p class="hour-time">{{ hour.time }}</p>
          <div class="hour-meta">
            <span class="meta-item">🔢 排序: {{ hour.sortOrder }}</span>
            <span class="meta-item">📅 {{ formatDate(hour.createTime) }}</span>
          </div>
        </div>
        <div class="hour-status">
          <span class="status-badge" :class="{ active: hour.status === 1 }">
            {{ hour.status === 1 ? '启用' : '禁用' }}
          </span>
        </div>
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <input 
              ...
              v-model="form.day" 
              class="form-input"
              placeholder="请输入星期（如：周一至周五、周六、周日）"
            />
          </div>
          ...
          <div class="form-group">
            ...
            <input 
              ...
              v-model="form.time" 
              class="form-input"
              placeholder="请输入时间（如：09:00 - 18:00）"
            />
          </div>
    ...
    <div class="modal-overlay" :class="{ show: showDeleteModal }" @click="closeDeleteModal">
      <div class="modal-container small" @click.stop>
        ...
        <div class="modal-body">
          <p>确定要删除工作时间 "<strong>{{ deletingHour?.day }}</strong>" 吗？</p>
...
const loadWorkingHours = async () => {
  try {
    const response = await request('/api/admin/working-hours?pageNum=1&pageSize=100')
    const data = await response.json()
    
    if (data.code === 200) {
      workingHours.value = data.data.records || []
    }
  } catch (error) {
...
const openEditModal = (hour) => {
  ...
  form.day = hour.day || ''
  form.time = hour.time || ''
  form.sortOrder = hour.sortOrder || 0
  form.status = hour.status ?? 1
  showModal.value = true
}
...
const handleSubmit = async () => {
  ...
  try {
    const url = isEdit.value 
      ? `/api/admin/working-hours/${form.id}` 
...
const confirmDelete = (hour) => {
  deletingHour.value = hour
...
const handleDelete = async () => {
  ...
  try {
    const response = await request(`/api/admin/working-hours/${deletingHour.value.id}`, {
      method: 'DELETE'
    })
...
const toggleStatus = async (hour) => {
  const newStatus = hour.status === 1 ? 0 : 1
  
  try {
    const response = await request(`/api/admin/working-hours/${hour.id}`, {
      method: 'PUT',
      body: JSON.stringify({
        status: newStatus
      })
