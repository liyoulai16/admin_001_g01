  ...
  <div class="forum-category-management">
      ...
      <div v-for="category in categories" :key="category.id" class="category-card">
        ...
        <div class="category-info">
          ...
          <div class="category-meta">
            <span class="meta-item">📊 帖子数量: {{ category.postCount || 0 }}</span>
    ...
    <div class="modal-overlay" :class="{ show: showModal }" @click="closeModal">
      <div class="modal-container" @click.stop>
        <div class="modal-header">
          <h2>{{ isEdit ? '编辑论坛版块' : '添加论坛版块' }}</h2>
          <button class="close-btn" @click="closeModal">×</button>
        </div>
        
        <div class="modal-body">
          <div class="form-group">
            <label class="form-label">版块名称 <span class="required">*</span></label>
            <input 
              type="text" 
              v-model="form.name" 
              class="form-input"
    ...
    <div class="modal-overlay" :class="{ show: showDeleteModal }" @click="closeDeleteModal">
      <div class="modal-container small" @click.stop>
        ...
        <div class="modal-body">
          <p>确定要删除论坛版块 "<strong>{{ deletingCategory?.name }}</strong>" 吗？</p>
          <p class="warning-text">此操作不可撤销，且版块下不能有帖子</p>
        </div>
        <div class="modal-footer">
...
const loadCategories = async () => {
  ...
  try {
    const response = await request('/api/admin/forum-categories?pageNum=1&pageSize=100')
...
const handleDelete = async () => {
  ...
  try {
    const response = await request(`/api/admin/forum-categories/${deletingCategory.value.id}`, {
      method: 'DELETE'
    })
...
const toggleStatus = async (category) => {
  ...
  try {
    const response = await request(`/api/admin/forum-categories/${category.id}/status?status=${newStatus}`, {
      method: 'PUT'
    })
