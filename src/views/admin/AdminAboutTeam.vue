  ...
  <div class="team-management">
    ...
    <div class="team-list" v-if="teams.length > 0">
      <div v-for="team in teams" :key="team.id" class="team-item">
        <div class="team-avatar">
          <span class="avatar-text">{{ team.avatar || '👨‍💼' }}</span>
        </div>
        <div class="team-info">
          <h3 class="team-name">{{ team.name }}</h3>
          <p class="team-role">{{ team.role }}</p>
          <p class="team-bio" v-if="team.bio">{{ team.bio }}</p>
          <div class="team-meta">
            <span class="meta-item">🔢 排序: {{ team.sortOrder }}</span>
            <span class="meta-item">📅 {{ formatDate(team.createTime) }}</span>
          </div>
        </div>
        <div class="team-status">
          <span class="status-badge" :class="{ active: team.status === 1 }">
            {{ team.status === 1 ? '启用' : '禁用' }}
          </span>
        </div>
        <div class="team-actions">
          <button class="action-btn edit" @click="openEditModal(team)">✏️ 编辑</button>
          <button 
            class="action-btn toggle" 
            :class="team.status === 1 ? 'disable' : 'enable'"
            @click="toggleStatus(team)"
          >
            {{ team.status === 1 ? '❌ 禁用' : '✅ 启用' }}
          </button>
          <button class="action-btn delete" @click="confirmDelete(team)">🗑️ 删除</button>
        </div>
      </div>
    </div>
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <input 
              ...
              v-model="form.avatar" 
              class="form-input"
              placeholder="请输入头像emoji（如：👨‍💼、👩‍💻）"
            />
            <div class="avatar-preview" v-if="form.avatar">
              <span class="preview-avatar">{{ form.avatar }}</span>
    ...
    <div class="modal-overlay" :class="{ show: showDeleteModal }" @click="closeDeleteModal">
      <div class="modal-container small" @click.stop>
        ...
        <div class="modal-body">
          <p>确定要删除团队成员 "<strong>{{ deletingTeam?.name }}</strong>" 吗？</p>
...
const loadTeams = async () => {
  try {
    const response = await request('/api/admin/about-teams?pageNum=1&pageSize=100')
    const data = await response.json()
    
    if (data.code === 200) {
      teams.value = data.data.records || []
    }
  } catch (error) {
...
const openEditModal = (team) => {
  ...
  isEdit.value = true
  form.id = team.id
  form.avatar = team.avatar || ''
  form.name = team.name || ''
  form.role = team.role || ''
  form.bio = team.bio || ''
  form.sortOrder = team.sortOrder || 0
  form.status = team.status ?? 1
  showModal.value = true
}
...
const handleSubmit = async () => {
  ...
  try {
    const url = isEdit.value 
      ? `/api/admin/about-teams/${form.id}` 
      : '/api/admin/about-teams'
...
const handleDelete = async () => {
  ...
  try {
    const response = await request(`/api/admin/about-teams/${deletingTeam.value.id}`, {
      method: 'DELETE'
    })
...
const toggleStatus = async (team) => {
  ...
  try {
    const response = await request(`/api/admin/about-teams/${team.id}`, {
      method: 'PUT',
      body: JSON.stringify({
        status: newStatus
      })
