  ...
  <div class="feature-management">
      ...
      <div v-for="feature in features" :key="feature.id" class="feature-item">
        ...
        <div class="feature-info">
          ...
          <div class="feature-meta">
            <span class="meta-item">🔢 排序: {{ feature.sortOrder }}</span>
            <span class="meta-item">📅 {{ formatDate(feature.createTime) }}</span>
          </div>
        </div>
        <div class="feature-status">
          <span class="status-badge" :class="{ active: feature.status === 1 }">
            {{ feature.status === 1 ? '启用' : '禁用' }}
          </span>
        </div>
        <div class="feature-actions">
          <button class="action-btn edit" @click="openEditModal(feature)">✏️ 编辑</button>
          <button 
            class="action-btn toggle" 
            :class="feature.status === 1 ? 'disable' : 'enable'"
            @click="toggleStatus(feature)"
          >
            {{ feature.status === 1 ? '❌ 禁用' : '✅ 启用' }}
          </button>
          <button class="action-btn delete" @click="confirmDelete(feature)">🗑️ 删除</button>
        </div>
      </div>
    </div>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input 
              ...
              class="form-input"
              placeholder="请输入描述（如：平均15分钟内回复您的咨询）"
            />
          </div>
    ...
    <div class="modal-overlay" :class="{ show: showDeleteModal }" @click="closeDeleteModal">
      <div class="modal-container small" @click.stop>
        ...
        <div class="modal-body">
          <p>确定要删除特色优势 "<strong>{{ deletingFeature?.title }}</strong>" 吗？</p>
...
const deletingFeature = ref(null)
...
const loadFeatures = async () => {
  try {
    const response = await request('/api/admin/features?pageNum=1&pageSize=100')
    const data = await response.json()
    
    if (data.code === 200) {
      features.value = data.data.records || []
    }
  } catch (error) {
    console.error('加载特色优势失败:', error)
  }
}
...
const openEditModal = (feature) => {
  ...
  form.sortOrder = feature.sortOrder || 0
  form.status = feature.status ?? 1
  showModal.value = true
}
...
const handleDelete = async () => {
  if (!deletingFeature.value) return
  
  isDeleting.value = true
  
  try {
    const response = await request(`/api/admin/features/${deletingFeature.value.id}`, {
      method: 'DELETE'
    })
...
const toggleStatus = async (feature) => {
  ...
  try {
    const response = await request(`/api/admin/features/${feature.id}`, {
      method: 'PUT',
      body: JSON.stringify({
        status: newStatus
      })
